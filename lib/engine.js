...
board[lt]     $T x $H x $D    "Levý bok"    at 0, 0, 0              color #9a6235
board[rt]     $T x $H x $D    "Pravý bok"   at {lt.right}+$W, 0, 0  color #9a6235
...
board[la]  LEN(0,0,200,$H) x $T x $D  "Left leg"   from 0,0 to 200,$H
board[ra]  LEN($W,0,200,$H) x $T x $D  "Right leg"  from $W,0 to 200,$H
...
board[br]   LEN($T,100,$W-$T,{ll.top}-100) x $T x $T "Brace"  from $T,100 to $W-$T,{ll.top}-100  color #8b6035` },
  ...
  parseBoard(line, ln) {
    ...
    try {
      ...
      if (viewPos !== -1) {
        ...
        const vch = viewStr.charAt(0).toLowerCase();
        if ('fst'.includes(vch)) view = vch;
        else throw new Error(`Neplatný pohled '${viewStr}', povoleno: f, s, t`);
      }
